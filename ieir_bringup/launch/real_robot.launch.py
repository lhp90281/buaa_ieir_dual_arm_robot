#!/usr/bin/env python3
"""
Control-side launch for the IEIR dual 7-DoF robot on REAL hardware.
...
------------------
  ...
  ros2 launch ieir_bringup real_robot.launch.py controller:=joint_position

  # Joint-space PD tracking with the local web console:
  ros2 launch ieir_bringup real_robot.launch.py \
       controller:=joint_position use_web:=true web_allow_control:=true

  # Gravity-comp teach mode with the local web console:
  ros2 launch ieir_bringup real_robot.launch.py use_web:=true

  # Both arms with the cartesian PD coordinator:
  ros2 launch ieir_bringup real_robot.launch.py controller:=cartesian_position

  # Single-arm bring-up (e.g. left only -- can0):
  ros2 launch ieir_bringup real_robot.launch.py arms:=left

  # No gripper (e.g. swapped end-effector):
  ros2 launch ieir_bringup real_robot.launch.py gripper:=false
...
----------------
  ...
  use_web        true | false   (start attach-only local web console).
  use_gui        compatibility alias for use_web; no MuJoCo panel is started.
  web_port       local HTTP port, default 8766.
  web_allow_control  true | false (offer explicit page controls; default false).
"""
...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument(
            ...
            description=(
                ...
                'profiles. Empty uses the ieir_controllers default.'
            ),
        ),
        DeclareLaunchArgument(
            'use_gui',
            default_value='true',
            description='Compatibility default for use_web; now starts Web UI, never MuJoCo',
            choices=['true', 'false'],
        ),
        DeclareLaunchArgument(
            'use_web', default_value=LaunchConfiguration('use_gui'),
            description='Start the local attach-only Web UI alongside controllers',
            choices=['true', 'false'],
        ),
        DeclareLaunchArgument('web_port', default_value='8766'),
        DeclareLaunchArgument('web_workspace', default_value=str(Path.cwd()),
                              description='Workspace containing src/description'),
        DeclareLaunchArgument('web_allow_control', default_value='false', choices=['true', 'false']),
        DeclareLaunchArgument(
            'teleop',
            default_value='false',
    ...
    # in a separate terminal via `ros2 launch ieir_bringup bridge.launch.py`.
    control_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                FindPackageShare('ieir_controllers'),
                'launch',
                'dual_arm.launch.py',
            ]),
        ),
    ...
    web_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            PathJoinSubstitution([
                FindPackageShare('ieir_bringup'),
                'launch',
                'web_console.launch.py',
            ]),
        ),
        launch_arguments={
            'workspace': LaunchConfiguration('web_workspace'),
            'port': LaunchConfiguration('web_port'),
            'allow_control': LaunchConfiguration('web_allow_control'),
            'gripper': LaunchConfiguration('gripper'),
            'preview': 'false',
        }.items(),
    ...
    return LaunchDescription([*args, control_launch, web_launch, teleop_launch])
