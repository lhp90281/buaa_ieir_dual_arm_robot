...
class DirectionPreview:
    ...
    def __init__(self, node, cfg, limit_review=False, motion_workflow=False, backend='web'):
        self.node, self.cfg = node, cfg
        self.backend = backend
        self.stage = 'limits' if limit_review else 'manual' if motion_workflow else 'direction'
        ...
        self.metadata_pub = node.create_publisher(String, self.prefix + '/metadata', 10)
        self.key_sub = node.create_subscription(
            String, self.prefix + '/key', self.on_key, 10)
        self.process = None if backend == 'web' else subprocess.Popen([
            'ros2', 'run', 'ieir_simulation', 'calibration_viewer', '--ros-args',
    ...
    def on_key(self, msg):
        if msg.data == 'ui_heartbeat':
            self.last_heartbeat = time.monotonic()
        else:
            self.keys.put(msg.data)

    def metadata(self, **fields):
        self.metadata_pub.publish(String(data=json.dumps(dict(
            backend=self.backend, stage=self.stage, channel=self.cfg.channel, **fields))))

    def is_closed(self):
        if self.process is not None:
            return self.process.poll() is not None
        return time.monotonic() - self.last_heartbeat > 3.0
    ...
    def wait_ready(self, stop_evt):
        ...
        while not stop_evt.is_set() and time.monotonic() < deadline:
            ...
            if self.backend == 'web':
                self.metadata(phase='waiting_for_browser')
                self.text_pub.publish(String(data='Waiting for Web UI; no motors enabled.'))
                # Direction mode already has a background executor; the other
                # two workflows must service the heartbeat while waiting here.
                if self.stage in ('limits', 'manual'):
                    rclpy.spin_once(self.node, timeout_sec=0.01)
            if (self.pose_pub.get_subscription_count() and self.text_pub.get_subscription_count()
                    and (self.backend != 'web' or not self.is_closed())):
                return
            time.sleep(0.05)
    ...
    def run(self, stop_evt):
        ...
        while not stop_evt.is_set() and not check.done:
            if self.is_closed():
                stop_evt.set()
                break
            if self.backend == 'web' and not self.node.exclusive():
                raise RuntimeError('Calibration command ownership lost')
            ...
            self.metadata(joint=name, angle_deg=math.degrees(angle), phase='direction')
    ...
    def review_limits(self, stop_evt):
        ...
        while not stop_evt.is_set() and not review.done:
            ...
            msg.header.stamp = self.node.get_clock().now().to_msg()
            self.pose_pub.publish(msg)
            self.metadata(joint=joint.name, angle_deg=math.degrees(angle), phase='review')
            self.text_pub.publish(String(data=(
