...
def motion_limits(args, joint, manual):
    ...
    if args.cal_kp is None:
        if args.gains_yaml is None:
            ...
            path = Path(get_package_share_directory('ieir_controllers')) / 'config/teleop_joint_gains.yaml'
        else:
...
def load_calibration_friction(args, joint, manual):
    ...
    from ament_index_python.packages import get_package_share_directory
    config = args.controller_config or Path(get_package_share_directory('ieir_controllers')) / 'config/dual_arm_controllers.yaml'
...
def main(argv=None):
    ...
    parser.add_argument('--preview-backend', choices=('web', 'mujoco'), default='web')
    ...
    parser.add_argument('--diagnostic-dir', type=Path,
                        default=Path('/tmp/ieir_calibration_diagnostics'),
...
def run_single(args):
    ...
    try:
        preview = DirectionPreview(node, cfg, motion_workflow=True, backend=args.preview_backend)
        ...
        while rclpy.ok() and not stop.is_set():
            ...
            if preview.is_closed():
                if manual_saved and preview.process is not None and preview.process.returncode == 0:
                    exit_requested = True
                    break
            ...
            if now - last_preview > 0.04:
                ...
                preview.metadata(joint=joint.name, angle_deg=math.degrees(joint.urdf_pos_at_limit), phase=state)
