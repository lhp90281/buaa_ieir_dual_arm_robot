...
Modes (CLI flag --mode):
    ...
    direction: FK-only Web preview, model zero plus one motor's delta.
...
class CalibrationNode(Node):
    ...
    def __init__(self, channel: int, joints: List[JointSpec], web_guard=False):
    ...
    def _tick_cmd(self) -> None:
        if self.web_guard and not self.exclusive():
            return
    ...
    def _publish_enable_msg(self, enable: bool) -> None:
        if enable and self.web_guard and not self.exclusive():
            raise RuntimeError('Calibration command ownership lost before enable')
    ...
    def exclusive(self):
        return (self.count_publishers('/w3_robot_bridge_node/commands') == 1
                and self.count_publishers('/w3_robot_bridge_node/command') == 0
                and self.count_publishers('/w3_robot_bridge_node/state') == 1)
...
def review_limit_references(cfg, input_path, reviewed_path, stop_evt, backend='web'):
    ...
    preview = None
    try:
        preview = DirectionPreview(node, cfg, limit_review=True, backend=backend)
...
def parse_args(argv=None):
    ...
    p.add_argument('--mode', default='hard-stop',
                   ...
                   help='hard-stop: per-joint push to single-side limit. '
                        ...
                        'one-shot capture of all joints. direction: Web incremental direction check. '
    ...
    p.add_argument('--preview-backend', choices=('web', 'mujoco'), default='web')
...
def main(argv=None) -> int:
    ...
    if args.mode in ('hard-stop', 'limit-preview'):
        ...
        try:
            if not review_limit_references(cfg, in_path, reviewed_path, stop_evt, args.preview_backend):
                rclpy.shutdown()
                return 130
    ...
    node = CalibrationNode(channel=cfg.channel, joints=cfg.joints, web_guard=args.preview_backend == 'web')
    exec_.add_node(node)
    ...
    if not stop_evt.is_set():
        try:
            if args.mode == 'direction':
                ...
                preview = DirectionPreview(node, cfg, backend=args.preview_backend)
                preview.wait_ready(stop_evt)
