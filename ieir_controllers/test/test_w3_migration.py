...
def test_gripper_model_changes_mass_not_arm_geometry():
    ...
    xacro = str(ROOT / 'ieir_controllers/config/dual_arm_ros2_control.urdf.xacro')
...
def test_single_arm_bridge_preserves_channel_indices(arms, gripper):
    ...
    robot = load('ieir_controllers/launch/dual_arm.launch.py')
    xml = robot.build_robot_description(
        str(ROOT / 'ieir_controllers/config/dual_arm_ros2_control.urdf.xacro'),
...
def test_calibration_filters_channels_offline_and_nonfinite():
    mod = load('ieir_controllers/scripts/joint_zero_calibration.py')
    node = object.__new__(mod.CalibrationNode)
    node.channel = 1
    node.web_guard = False
...
def test_disabled_grippers_do_not_publish_teleop_commands():
    mod = load('ieir_controllers/scripts/teleop_joint_bridge.py')
...
def test_direction_preview_uses_only_delta_and_requires_motion():
    mod = load('ieir_controllers/scripts/calibration_direction.py')
...
def test_direction_file_and_signed_hard_stop_formula():
    mod = load('ieir_controllers/scripts/calibration_direction.py')
    zero = load('ieir_controllers/scripts/joint_zero_calibration.py')
