...
void UiEvent(mjuiState* state) {
  ...
  // IEIR panel shortcuts must run before MuJoCo's built-in UI event
  ...
  if (state->type==mjEVENT_KEY && state->key!=0 && !state->control && !state->alt) {
    ...
    case 'J':
    case 'j':
      sim->ieir_panel_joint_mode_request.store(true);
      return;

    case 'G':
    case 'g':
      sim->ieir_panel_gravity_mode_request.store(true);
      return;

    case 'K':
    case 'k':
      sim->ieir_panel_cartesian_mode_request.store(true);
      return;

    case 'H':
    case 'h':
      sim->ieir_panel_home_request.store(true);
      return;

    case 'E':
    case 'e':
      sim->ieir_panel_start_edit_request.store(true);
      return;

    case 'S':
    case 's':
      sim->ieir_panel_send_target_request.store(true);
      return;

    case 'C':
    case 'c':
      sim->ieir_panel_cancel_edit_request.store(true);
      return;

    case 'T':
    case 't':
      sim->ieir_panel_teleop_prepare_request.store(true);
      return;

    case 'Y':
    case 'y':
      sim->ieir_panel_teleop_toggle_request.store(true);
      return;

    case 'U':
    case 'u':
      sim->ieir_panel_teleop_exit_request.store(true);
      return;
    }
  }
